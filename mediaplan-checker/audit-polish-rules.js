  ...
  function addSpecificTextRoots(workbook, issues) {
    for (const sheet of workbook.SheetNames || []) {
      ...
      for (const e of entries(ws)) {
        ...
        if (/размещен\w*(?:\s+\S+){0,10}\s+размещен\w*/iu.test(raw)) {
          const item = issue(
            'Возможная тавтология', sheet, e.addr, raw,
            'В коротком фрагменте повторяется слово/корень «размещ…».',
            'Формулировка может выглядеть тяжело и тавтологично в клиентском тексте.',
            'Переформулируйте фразу короче, сохранив смысл.'
          );
