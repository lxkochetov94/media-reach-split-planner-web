...
old_top=r'''class="metric ${result.difference?'diff-neg':'diff-pos'}">${result.difference>=0?'+':''}${money(result.difference)}'''
new_top=r'''class="metric ${result.overallRoundingOnly?'mp-rounding-value':(result.difference?'diff-neg':'diff-pos')}">${displayDifference>=0?'+':''}${money(displayDifference)}'''
if old_top not in app:
    raise SystemExit('top difference display anchor missing')
app=app.replace(old_top,new_top,1)

old_channel=r'''${x.excluded?'<span class="mp-excluded-pill">Исключено</span>':x.roundingOnly?'0 ₽':`${x.difference>=0?'+':''}${money(x.difference)}`}'''
new_channel=r'''${x.excluded?'<span class="mp-excluded-pill">Исключено</span>':x.roundingOnly?`${(x.rawDifference??0)>=0?'+':''}${money(x.rawDifference??0)}`:`${x.difference>=0?'+':''}${money(x.difference)}`}'''
...
old_footer=r'''class="money ${result.difference?'diff-neg':'diff-pos'}">${result.difference>=0?'+':''}${money(result.difference)}'''
new_footer=r'''class="money ${result.overallRoundingOnly?'mp-rounding-value':(result.difference?'diff-neg':'diff-pos')}">${displayDifference>=0?'+':''}${money(displayDifference)}'''
if old_footer not in app:
    raise SystemExit('footer difference display anchor missing')
app=app.replace(old_footer,new_footer,1)

old_detail=r'''${d.roundingOnly?'0 ₽':`${d.difference>=0?'+':''}${money(d.difference||0)}`}'''
new_detail=r'''${d.roundingOnly?`${(d.rawDifference??0)>=0?'+':''}${money(d.rawDifference??0)}`:`${d.difference>=0?'+':''}${money(d.difference||0)}`}'''
